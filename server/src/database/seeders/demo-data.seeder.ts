...
export const seedDemoData = async (): Promise<void> => {
  ...
  try {
    ...
    // 5.5 Seed Epics
    console.log('⚡ Creating project epics...');
    const wolfEpics = await Epic.bulkCreate(
      [
        {
          project_id: projectWolf.id,
          name: 'Estate Intake & Questionnaires',
          description: 'Client onboarding questionnaires, family trees, and automated asset listings',
          color: '#8b5cf6',
          status: EpicStatus.IN_PROGRESS,
        },
        {
          project_id: projectWolf.id,
          name: 'Beneficiary & Asset Distribution',
          description: 'Complex asset distribution rules, percentage allocation, and secondary beneficiaries',
          color: '#06b6d4',
          status: EpicStatus.TODO,
        },
        {
          project_id: projectWolf.id,
          name: 'Document Notarization Engine',
          description: 'Automated legal PDF generation, digital signature verification, and audit stamps',
          color: '#10b981',
    ...
    const appEpics = await Epic.bulkCreate(
      [
        {
          project_id: projectApp.id,
          name: 'Staff Access Controls & RBAC',
          description: 'Granular role-based permissions, session expiration, and audit logging',
          color: '#f59e0b',
          status: EpicStatus.IN_PROGRESS,
        },
        {
          project_id: projectApp.id,
          name: 'Payment Auditing & Compliance',
          description: 'Stripe webhook verification, automated invoice reconciliation, and tax reports',
          color: '#ec4899',
          status: EpicStatus.TODO,
        },
      ],
    ...
    const [epicIntake, epicDist, epicNotary] = wolfEpics;
    const [epicRbac, epicAudit] = appEpics;
