...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div className="w-80 shrink-0 bg-slate-100/85 border border-slate-200/90 rounded-2xl flex flex-col max-h-full shadow-2xs">
      {/* Column Header */}
      <div className="p-3.5 pb-2.5 flex items-center justify-between border-b border-slate-200/70">
        <div className="flex items-center gap-2">
          <span className={`w-2.5 h-2.5 rounded-full ${getColumnDotColor(column.name)}`} />
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">
            {column.name}
          </h3>
          <span className="px-2 py-0.5 rounded-full bg-white border border-slate-200 text-[11px] font-bold text-slate-600 shadow-2xs">
            {issues.length}
          </span>
        </div>
        ...
        <button
          ...
          className="p-1 rounded-lg text-slate-400 hover:text-slate-800 hover:bg-slate-200/70 transition-colors cursor-pointer"
